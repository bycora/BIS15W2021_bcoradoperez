...
```r
n_mammals<-select(mammals, "family","genus","species","mass","gestation")
n_mammals
```
...
```r
filter(n_mammals, family=="Bovidae" & mass>450000)
```
...
```r
smol_bean<-filter((select(primate_genera, "genus","species", "mass", "gestation", "newborn")), newborn <=400)
smol_bean
```
...
```
#The Macaca genus, Maura species, has the smallest newborn mass of 389.50 kg(units not specified).
